import { PublicKey } from '@solana/web3.js';

export const _MARKETS = [
  {
    name: 'NORA/USDC',
    deprecated: false,
    address: new PublicKey('5JC2kckdehvq8k2JMZE9gJzdiyhGyg5CH4HN8gxwWSyo'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },
  {
    name: 'SOL/USDC',
    deprecated: false,
    address: new PublicKey('9wFFyRfZBsuAha4YcuxcXLKwMxJR43S7fPfQLusDBzvT'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },
  {
    name: 'RAY/USDC',
    deprecated: false,
    address: new PublicKey('teE55QrL4a4QSfydR9dnHF97jgCfptpuigbb53Lo95g'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },
  {
    name: 'SOL/USDT',
    deprecated: false,
    address: new PublicKey('HWHvQhFmJB3NUcu1aihKmrKegfVxBEHzwVX6yZCKEsi1'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },
  {
    name: 'BTC/USDC',
    deprecated: false,
    address: new PublicKey('A8YFbxQYFVqKZaoYJLLUVcQiWP7G2MeEgW5wsAQgMvFw'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },
  {
    name: 'WETH/USDT',
    deprecated: false,
    address: new PublicKey('ch7kmPrtoQUSEPBggcNAvLGiMQkJagVwd3gDYfd8m7Q'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },
  {
    name: 'SRM/USDC',
    deprecated: false,
    address: new PublicKey('ByRys5tuUWDgL73G8JBAEfkdFf8JWBzPBDHsBVQ5vbQA'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },
  {
    name: 'WMATICPO/USDC',
    deprecated: false,
    address: new PublicKey('5WRoQxE59966N2XfD2wYy1uhuyKeoVJ9NBMH6r6RNYEF'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },
  {
    name: 'WLINK/USDC',
    deprecated: false,
    address: new PublicKey('FJMjxMCiDKn16TLhXUdEbVDH5wC6k9EHYJTcrH6NcbDE'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },
  {
    name: 'RAY/USDC',
    deprecated: false,
    address: new PublicKey('6UmmUiYoBjSrhakAobJw8BvkmJtDVxaeBtbt7rxWo1mg'),
    programId: new PublicKey('675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8'),
  },
  {
    name: 'WBNB/USDC',
    deprecated: false,
    address: new PublicKey('4UPUurKveNEJgBqJzqHPyi8DhedvpYsMXi7d43CjAg2f'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },{
    name: 'WAVAX/USDT',
    deprecated: false,
    address: new PublicKey('E8JQstcwjuqN5kdMyUJLNuaectymnhffkvfg1j286UCr'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },{
    name: 'SAMO/USDT',
    deprecated: false,
    address: new PublicKey('FR3SPJmgfRSKKQ2ysUZBu7vJLpzTixXnjzb84bY3Diif'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },{
    name: 'SLND/USDT',
    deprecated: false,
    address: new PublicKey('F9y9NM83kBMzBmMvNT18mkcFuNAPhNRhx7pnz9EDWwfv'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },{
    name: 'FIDA/USDT',
    deprecated: false,
    address: new PublicKey('E14BKBhDWD4EuTkWj1ooZezesGxMW8LPCps4W5PuzZJo'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },{
    name: 'WBNB/USDT',
    deprecated: false,
    address: new PublicKey('FjbKNZME5yVSC1R3HJM99kB3yir3q3frS5MteMFD72sV'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },
  {
    name: 'WFTT/USDC',
    deprecated: false,
    address: new PublicKey('2wteg25ch227n4Rh1CN4WNrDZXBpRBpWJ48mEC2K7f4r'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },
  {
    name: 'KIN/USDC',
    deprecated: false,
    address: new PublicKey('Bn6NPyr6UzrFAwC4WmvPvDr2Vm8XSUnFykM2aQroedgn'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },
  {
    name: 'MNGO/USDC',
    deprecated: false,
    address: new PublicKey('3d4rzwpy9iGdCZvgxcu7B1YocYffVLsQXPXkBZKt2zLc'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },
  {
    name: 'WUNI/USDC',
    deprecated: false,
    address: new PublicKey('B7b5rjQuqQCuGqmUBWmcCTqaL3Z1462mo4NArqty6QFR'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  },
 {
    name: 'CATO/USDC',
    deprecated: false,
    address: new PublicKey('9fe1MWiKqUdwift3dEpxuRHWftG72rysCRHbxDy6i9xB'),
    programId: new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin'),
  }
];
